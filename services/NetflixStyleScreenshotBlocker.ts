...
class NetflixStyleScreenshotBlocker {
  ...
  private detectionCooldown = 3000; // 3 seconds cooldown between detections
  ...
  async enableProtection(): Promise<void> {
    try {
      if (this.isProtectionEnabled) {
        console.log('🔒 Netflix-style screenshot blocker already enabled');
        return;
      }

      this.isProtectionEnabled = true;
      this.isBlockingActive = true;
      console.log('🔒 Netflix-style screenshot blocker enabled');
      ...
      console.error('❌ Error enabling Netflix-style screenshot blocker:', error);
      throw error;
    }
  }

  /**
   * Disable Netflix-style screenshot blocking
   */
  async disableProtection(): Promise<void> {
    try {
      if (!this.isProtectionEnabled) {
        console.log('🔓 Netflix-style screenshot blocker already disabled');
        return;
      }
      ...
      console.log('🔓 Netflix-style screenshot blocker disabled');
      ...
      console.error('❌ Error disabling Netflix-style screenshot blocker:', error);
      throw error;
    }
  }
  ...
  private setupAppStateListener(): void {
    this.appStateSubscription = AppState.addEventListener('change', async (nextAppState: AppStateStatus) => {
      ...
      if (nextAppState === 'active' && this.isProtectionEnabled) {
        try {
          console.log('🔒 Netflix-style screenshot blocker re-enabled on app focus');
          // Restart protection monitoring when app becomes active
          this.startProtectionMonitoring();
        } catch (error) {
          console.error('❌ Error re-enabling Netflix-style screenshot blocker:', error);
        }
  ...
  private startProtectionMonitoring(): void {
    ...
    }, 200); // Check every 200ms for ultra-sensitive detection

    console.log('🛡️ Netflix-style protection monitoring started');
  }
  ...
  private stopProtectionMonitoring(): void {
    if (this.protectionInterval) {
      ...
      console.log('🛡️ Netflix-style protection monitoring stopped');
    }
  }
  ...
  private showProtectionWarning(): void {
    ...
    setTimeout(() => {
      Alert.alert(
        ...
        'This app uses advanced screenshot detection and blocking. Screenshots will show a black screen like Netflix.',
        [
          {
            text: 'I Understand',
            style: 'default',
            onPress: () => {
              console.log('✅ User acknowledged Netflix-style screenshot protection');
            }
          }
        ],
        { cancelable: false }
  ...
  showWarning(): void {
    Alert.alert(
      '🚫 Screenshot Blocked',
      'Netflix-style screenshot protection is active. Your screenshot attempt has been detected and blocked with a black screen.',
      [
        {
          text: 'OK',
          style: 'default'
  ...
  async cleanup(): Promise<void> {
    try {
      ...
      this.isBlockingActive = false;
      this.blackScreenActive = false;
      console.log('🧹 Netflix-style screenshot blocker service cleaned up');
    } catch (error) {
      console.error('❌ Error cleaning up Netflix-style screenshot blocker service:', error);
    }
  }
}

// Export singleton instance
export default new NetflixStyleScreenshotBlocker();
