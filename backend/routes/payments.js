...
// Treat missing or placeholder Paymob credentials as "not configured"
function isPaymobConfigured() {
  const key = (process.env.PAYMOB_API_KEY || '').trim();
  const integrationId = String(process.env.PAYMOB_INTEGRATION_ID_CARD || '').trim();
  if (!key || !integrationId) return false;
  ...
  const placeholderIds = new Set(['123456', '0', 'changeme', 'xxx', 'test']);

  if (placeholderKeys.has(key.toLowerCase()) || placeholderIds.has(integrationId)) {
    return false;
  }
...
router.post('/create-order', requireAuth, async (req, res) => {
  try {
    ...
    // Check Paymob configuration - Test Mode (dev + missing/placeholder keys)
    const paymobReady = isPaymobConfigured();
    const isTestMode = process.env.NODE_ENV === 'development' && !paymobReady;
    
    console.log('🔍 NODE_ENV:', process.env.NODE_ENV);
    console.log('🔍 PAYMOB configured:', paymobReady);
    ...
    if (isTestMode) {
      ...
      try {
        ...
        // Create subscription record (test mode) — active so unlock works immediately
        ...
        console.log('✅ Test subscription created (ACTIVE):', subscription._id);

        // Return test payment data
        return res.json({
          status: 'success',
          message: 'Test mode: Subscription activated successfully (Paymob not configured)',
          data: {
            ...
            currency: 'EGP',
            paymentMethod,
            redirectUrl: `https://test-payment.elnadyapp.com?order=${subscription.paymobOrderId}`,
            testMode: true,
    ...
    if (!paymobReady) {
      console.error('❌ Paymob is not configured (missing or placeholder credentials)');
      return res.status(500).json({
        status: 'error',
        message: 'Payment service is not configured. Please contact support.',
        code: 'PAYMENT_SERVICE_NOT_CONFIGURED',
        details: 'Configure real PAYMOB_API_KEY and PAYMOB_INTEGRATION_ID_CARD in backend/config.env, or use Activate Test Premium in development.'
      });
    }
    ...
    // Temporary order id until Paymob returns a real one (field is required by schema)
    const pendingOrderId = `PENDING_${Date.now()}_${req.user.id}`;
    ...
    // Persist real Paymob order id on the subscription
...
// @route   POST /api/payments/activate-test
// @desc    Instantly activate a test subscription (for local unlock testing)
// @access  Private
router.post('/activate-test', requireAuth, async (req, res) => {
  try {
    ...
    const endDate = new Date();
    endDate.setDate(endDate.getDate() + (planDetails.duration || 30));

    // Reactivate a pending test sub if present, otherwise create one
    ...
    if (subscription) {
      ...
      subscription.paymobOrderId = subscription.paymobOrderId || `TEST_ORDER_${Date.now()}`;
      subscription.paymobTransactionId = subscription.paymobTransactionId || `TEST_TX_${Date.now()}`;
      await subscription.save();
    } else {
