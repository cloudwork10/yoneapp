...
const withMedia = (docs, fields, req) => {
  if (!docs) return docs;
  if (Array.isArray(docs)) {
    return docs.map((doc) => resolveMediaFields(doc, fields, req));
  }
...
router.get('/stats', async (req, res) => {
  try {
    const [courses, articles, roadmaps, podcasts, reels, advices, terms, cvs] = await Promise.all([
      ...
      Reel.countDocuments({ status: 'approved', isActive: true }),
    ...
    res.json({
      status: 'success',
      data: { courses, articles, roadmaps, podcasts, reels, advices, terms, cvs }
    });
  } catch (error) {
...
router.get('/articles', async (req, res) => {
  try {
    ...
    res.json({
      status: 'success',
      data: {
        articles: withMedia(articles, ['thumbnail', 'image'], req)
      }
    });
  } catch (error) {
...
router.get('/articles/:id', async (req, res) => {
  try {
    ...
    res.json({
      status: 'success',
      data: {
        article: withMedia(article, ['thumbnail', 'image'], req)
      }
    });
  } catch (error) {
...
router.get('/roadmaps', async (req, res) => {
  try {
    ...
    res.json({
      status: 'success',
      data: {
        roadmaps: withMedia(roadmaps, ['thumbnail', 'image'], req)
      }
    });
  } catch (error) {
...
router.get('/roadmaps/:id', async (req, res) => {
  try {
    ...
    res.json({
      status: 'success',
      data: {
        roadmap: withMedia(roadmap, ['thumbnail', 'image'], req)
      }
    });
  } catch (error) {
...
router.get('/content/advices', async (req, res) => {
  try {
    ...
    res.json({
      status: 'success',
      data: {
        advices: withMedia(advices, ['thumbnail', 'image'], req)
      }
    });
  } catch (error) {
