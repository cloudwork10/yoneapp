...
export default function SubscriptionScreen() {
  ...
  const handleActivateTestSubscription = async () => {
    ...
    try {
      setSubscribing(true);
      const response = await makeAuthenticatedRequest(`${API_BASE_URL}/api/payments/activate-test`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ plan: selectedPlan || 'monthly' }),
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) {
        Alert.alert('Failed', data.message || 'Could not activate test subscription');
        return;
      }
      await fetchCurrentSubscription();
      Alert.alert('Activated', 'Test premium subscription is now active. Locked content should open.');
    } catch (error) {
      console.error('Activate test subscription error:', error);
      Alert.alert('Error', 'Network error while activating test subscription');
    } finally {
      ...
      if (data.status === 'success' && data.data) {
        ...
        if (data.data.testMode) {
          Alert.alert(
            ...
            'Premium is now active on this account. Open a locked lesson/content to verify unlock.',
            [
              {
                text: 'OK',
                onPress: () => {
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <LinearGradient colors={['#000000', '#1a1a1a']} style={styles.container}>
        <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
          ...
          <TouchableOpacity
            style={styles.testUnlockButton}
            onPress={handleActivateTestSubscription}
            disabled={subscribing}
          >
            <Text style={styles.testUnlockButtonText}>🧪 Activate Test Premium</Text>
            <Text style={styles.testUnlockHint}>For unlock testing without Paymob</Text>
          </TouchableOpacity>
