...
export default function CoursesScreen() {
  ...
  const fetchCourses = async () => {
    try {
      // Only block list with skeleton on first load
      if (courses.length === 0) {
        setLoading(true);
      }
      setError(null);
      ...
      if (response.ok) {
        ...
        const fetchedCourses = data.data.courses.map((course: any) => ({
          ...
          thumbnail: resolveMediaUrl(
            ...
            'https://via.placeholder.com/300x200/1a1a1a/4ECDC4?text=Course+Image'
          ),
          ...
          {error && !loading ? (
            ...
            <>
              {Array.from({ length: 3 }, (_, index) => (
                <ArticleCardSkeleton key={`course-skeleton-${index}`} />
              ))}
            </>
          ) : (
