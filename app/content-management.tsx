...
function AccessTypePicker({
  ...
  value?: string;
  onChange: (next: AccessType) => void;
  fallback?: AccessType;
}) {
  const selected: AccessType = value === 'premium' || value === 'free' ? value : fallback;

  return (
    <View style={accessStyles.box}>
      <Text style={accessStyles.title}>الوصول للمحتوى</Text>
      <Text style={accessStyles.hint}>اضغط للاختيار • مجاني للكل أو بالاشتراك فقط</Text>
      <View style={accessStyles.row}>
        <TouchableOpacity
          activeOpacity={0.85}
          style={[accessStyles.btn, selected === 'free' && accessStyles.btnFreeOn]}
          onPress={() => onChange('free')}
        >
          <Text style={[accessStyles.btnTitle, selected === 'free' && accessStyles.btnTitleOn]}>
            مجاني
          </Text>
          <Text style={[accessStyles.btnSub, selected === 'free' && accessStyles.btnSubOn]}>
            {selected === 'free' ? '✓ مختار الآن' : 'يفتح بدون اشتراك'}
          </Text>
        </TouchableOpacity>

        <TouchableOpacity
          activeOpacity={0.85}
          style={[accessStyles.btn, selected === 'premium' && accessStyles.btnPremiumOn]}
          onPress={() => onChange('premium')}
        >
          <Text style={[accessStyles.btnTitle, selected === 'premium' && accessStyles.btnTitleOn]}>
            اشتراك
          </Text>
          <Text style={[accessStyles.btnSub, selected === 'premium' && accessStyles.btnSubOn]}>
            {selected === 'premium' ? '✓ مختار الآن' : 'مقفول لغير المشترك'}
          </Text>
        </TouchableOpacity>
      </View>
    </View>
                ...
                <ArticleForm 
                  ...
                  onSave={async (articleData) => {
                    try {
                      ...
                      if (response.ok) {
                        const result = await response.json();
                        const saved = result?.data?.article || result?.data;

                        if (editingArticle) {
                          setArticles(articles.map(article =>
                            article._id === editingArticle._id ? saved : article
                          ));
                        ...
                        Alert.alert('Error', errorData?.message || `Failed to save article (${response.status})`);
                      }
                    } catch (error) {
                ...
                <RoadmapForm 
                  ...
                  onSave={async (roadmapData) => {
                    try {
                      ...
                      if (response.ok) {
                        const result = await response.json();
                        const saved = result?.data?.roadmap || result?.data;

                        if (editingRoadmap) {
                          setRoadmaps(roadmaps.map(roadmap =>
                            roadmap._id === editingRoadmap._id ? saved : roadmap
                          ));
                ...
                <AdviceForm
                  ...
                  onSave={async (adviceData) => {
                    try {
                      ...
                      const response = await makeAuthenticatedRequest(url, {
                        method: editingAdvice ? 'PUT' : 'POST',
                        body: JSON.stringify(adviceData),
                      });

                      if (response.ok) {
                        const result = await response.json();
                        const saved = result?.data?.advice || result?.data;

                        if (editingAdvice) {
                          setAdvices(advices.map(advice =>
                            advice._id === editingAdvice._id ? saved : advice
                          ));
                        } else {
                          setAdvices([saved, ...advices]);
                          await NotificationService.sendContentNotification(
                            'advice',
                            saved?.title || 'نصيحة جديدة',
                            saved?.author
                          );
                        }
                        ...
                        const errorData = await response.json().catch(() => ({}));
                        Alert.alert('Error', errorData?.message || `Failed to save advice (${response.status})`);
                      }
                    } catch (error) {
                      Alert.alert('Error', `Network error: ${error.message}`);
                    }
...
const RoadmapForm = ({ roadmap, onSave, onCancel, knownCategories = [] }: { roadmap: any, onSave: (data: any) => void, onCancel: () => void, knownCategories?: string[] }) => {
  const [formData, setFormData] = useState({
    ...
    accessType: roadmap?.accessType || 'free',
  });
...
const AdviceForm = ({ advice, onSave, onCancel, knownCategories = [] }) => {
  const [formData, setFormData] = useState({
    ...
    accessType: advice?.accessType || 'free',
...
const adviceStyles = StyleSheet.create({
  ...
  accessChipFreeActive: {
    backgroundColor: 'rgba(52, 211, 153, 0.2)',
    borderColor: '#34D399',
  },
  accessChipPremiumActive: {
    backgroundColor: 'rgba(229, 9, 20, 0.2)',
    borderColor: '#E50914',
  },
  accessChipText: {
...
const TermForm = ({ term, onSave, onCancel, knownLanguages = [] }: { term: any, onSave: (data: any) => void, onCancel: () => void, knownLanguages?: string[] }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    accessType: term?.accessType || 'free',
  });
  ...
  const uploadAudio = async () => {
    try {
      ...
      const response = await makeAuthenticatedRequest(`${API_BASE_URL}/api/admin/content/upload-audio`, {
        method: 'POST',
        body: formDataUpload,
      });
